...
- 摄像头实时预览
- 单一摄像头画质列表，自动模式优先使用适合画中画的 720p / 30 fps
- 支持 480p、720p 与 1080p 画质档位，帧率由应用自动匹配
- 跟随摄像头、16:9、4:3、1:1 与自由调整等画中画比例
- 填满裁切或完整显示摄像头画面
- 直接拖动和缩放画中画
- 从菜单栏重置画中画位置
- 状态栏图标入口
...
4. 发现遮挡时，立刻拖动或缩放悬浮窗
5. 需要时可通过控制面板调整摄像头画质、画中画大小与比例、圆角和镜像

## 默认设置

- 摄像头画质：自动优先选择 720p / 30 fps，并在控制面板显示实际输入
- 画中画大小：中号，窗口宽度 320；使用 16:9 摄像头时尺寸为 320 × 180
- 画中画比例：跟随摄像头，避免用户手动判断画面比例
