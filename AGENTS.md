...
- 默认安装目标为：`/Applications/SameShot.app`
- 默认行为是用新打包出的 `dist/SameShot.app` 覆盖同步 `/Applications/SameShot.app`
- 除非用户明确指定其他安装位置，否则 `/Applications/SameShot.app` 视为最终交付版本

## 打包完成后的反馈

- 需要明确告知用户是否打包成功。
- 需要明确告知最终 App 路径是 `/Applications/SameShot.app`。
